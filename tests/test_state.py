...
class TestCodeFormat(unittest.TestCase):
    """
    A class to test pep8 on state file"""
    def test_pycodestyle(self):
        """
        Test pep8 format
        """
        pycostyle = pycodestyle.StyleGuide(quiet=True)
        result = pycostyle.check_files(['models/state.py'])
...
class Test_docstrings(unittest.TestCase):
    ...
    def setup_class(self):
        ...
        """
        self.obj_members(State, inspect.isfunction)

    def test_module_dostring(self):
        """
        Test for exist module docstrings
        """
        msg = "state.py file needs a docstrings"
        self.assertIsNotNone(models.state.__doc__, msg)
        self.assertTrue(len(__doc__) > 0, " state.py have docstrings")
        self.assertFalse(len(__doc__) < 0, " state  have docstrings")


class Test_Class_State(unittest.TestCase):
    """Testing State class"""
    @mock.patch('models.storage')
    def test_State_instance(self, mock_storage):
        instance = State()
        self.assertEqual(type(instance), State)
        self.assertTrue(type(instance) == State)
        ...
        self.assertEqual(dict_inst['name'], 'California')
        self.assertEqual(dict_inst['__class__'], 'State')
    ...
    def test_str_method(self):
        ...
        instance6 = State()
        st_id = instance6.id
        st_dict = instance6.__dict__
        string_output = "[State] ({}) {}".format(st_id, st_dict)
